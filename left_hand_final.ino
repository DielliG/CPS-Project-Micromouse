...
/*********** IR PINS *********/
const byte IRFL = D14;  // the IR Front sensor in left postion
// const byte IRFR = D15;  // the IR Front sensor in right postion

const byte IRSL = D13;  // the IR Front sensor in right postion
// const byte IRSR = D15;  // the IR Side sensor in right postion

/*********** ENCODER PINS *********/
const byte LEFT_ENCODER_A = PB5;  // hardware interrupt
const byte LEFT_ENCODER_B = PC6;
const byte RIGHT_ENCODER_A = PB4;  // hardware interrupt
const byte RIGHT_ENCODER_B = PC5;

/*********** MOTOR PINS *********/
const byte IN1 = 2;  // Left Motor Negative
const byte IN2 = 3;  // Left Motor Positive

const byte IN3 = 4;  // Right Motor Negative
const byte IN4 = 5;  // Right Motor Positive

const byte ENL = 9;  // Enable for Left motor
const byte ENR = 10;  // Enable for Right motor

/*********** ULTRASONIC  ***********/
const int LEFT_ECHO_PIN = 11;
const int LEFT_TRIG_PIN = 12;

const int RIGHT_ECHO_PIN = D7;
const int RIGHT_TRIG_PIN = D8;
...
void loop() {
  ...
  if (digitalRead(IRSL) == 1) {  // if the left is clear
    ...
    turn_left(80, 85);
    forward2(75, 225);
  } else if (digitalRead(IRFL) == 1 ) {  // if front is clear
    rightCount = 0;
    leftCount = 0;
    state = 0;
    forward(80);
...
void forward2(int mspeed, int steps) {
  ...
  while (digitalRead(IRFL) && (encoderCountR + encoderCountL) / 2 <= steps) {
    myPID(mspeed);
  }
  stopAll();
  delay(500);
...
void forward(int mspeed) {
  ...
  while (digitalRead(IRFL) && digitalRead(IRSL) == 0) {  // (encoderCountR + encoderCountL)/2 <=200
    myPID(mspeed);
  }
  if (digitalRead(IRSL)) {
    forward2(mspeed, 150);
...
//     }
//     if (state > 0) {
//       for (int i = 0; i < 25000 && digitalRead(IRFL); i++) {
//         digitalWrite(IN1, LOW);
//         digitalWrite(IN2, HIGH);
//         digitalWrite(IN3, LOW);
//         digitalWrite(IN4, HIGH);
